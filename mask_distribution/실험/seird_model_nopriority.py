...
def run_dynamic_experiment():
    ...
    # 개별 실험 조건 파일
    output_csv_path = os.path.join(output_dir, f'policy_results_without_priority_{exp_conditions}.csv')
    
    # 누적 마스터 파일
    master_csv_path = os.path.join(output_dir, 'policy_results_without_priority_accumulated.csv')
    ...
    header = ("Region1,Region2,Region1_NonAged,Region2_NonAged,Region1_Aged,Region2_Aged,Total_Mortality,"
              "policy,supply_rate,epsilon,R1_Masks,R2_Masks,"
    ...
    for policy_name, mortality_rates in results.items():
        ...
        line = (f"{mortality_rates['region1']:.6f},"
    ...
    if SAVE_INDIVIDUAL_CSV:
        with open(output_csv_path, 'w') as f:
            ...
            f.write("Region1,Region2,Region1_NonAged,Region2_NonAged,Region1_Aged,Region2_Aged,Total_Mortality,"
                    "policy,supply_rate,epsilon,R1_Masks,R2_Masks,Achieved_Utility,Total_Utility,Gini_Value\n")
            for line in results_data:
                # 실험 조건 컬럼 제외하고 쓰기 (utility 정보는 포함, 마지막 8개 실험조건 컬럼만 제외)
                f.write(','.join(line.split(',')[:15]) + '\n')
    ...
    if SAVE_INDIVIDUAL_CSV:
        print(f"SEIRD model (no priority) results saved to {output_csv_path}")
