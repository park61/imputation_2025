...
def solve_problem_gurobi(epsilon, supply, total_population, num_breaks, sub_population, slope, max_utility):
    ...
    mdl.addConstr(sum(d_plus[j, k] + d_minus[j, k] for j, k in B)  <= epsilon
                  * 2* sum(max_utility[i] for i in N) * sum(slope[i, t] * x[i, t] for i, t in A))
...
def get_gini_value(sub_population,slope,allocation,max_utility):
  ...
  temp_denominator_value = 2*sum(max_utility[i] for i in range(n))*sum(achieved_utility[i] for i in range(n))
...
def run_allocation_scenarios(df_results=None):
    ...
    # 실험 조건을 파일명에 포함
    pop_str = f"pop{conditions.populations[0]}_{conditions.populations[1]}"
    aged_str = f"aged{conditions.aged_ratio[0]}_{conditions.aged_ratio[1]}"
    slope_str = f"slope{conditions.slope_values[0][0]}_{conditions.slope_values[0][1]}_{conditions.slope_values[1][0]}_{conditions.slope_values[1][1]}"
    exp_conditions_str = f"{pop_str}_{aged_str}_{slope_str}"
    
    output_file_path = os.path.join(output_dir, f'df_results_output_{exp_conditions_str}.csv')
    
    if SAVE_INDIVIDUAL_CSV:
        df_results.to_csv(output_file_path, index=False)
        print(f"All allocation results saved to {output_file_path}")
    else:
        print(f"Individual file saving skipped (optimization mode)")
