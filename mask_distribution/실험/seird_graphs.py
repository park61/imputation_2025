...
from conditions import populations, slope_values, aged_ratio # populations, slope_values, aged_ratio 변수 import
import os
...
def plot_fair_policy_heatmap(df, graphs_dir):
    ...
    # 2. 노령층 인구비율은 conditions.py의 aged_ratio를 사용
...
def plot_fair_policy_by_epsilon(df, graphs_dir):
    ...
    for col, title in zip(mortality_columns, titles):
        ...
        filename_title = title.replace(' ', '_').replace(':', '')
        # 실험 조건을 파일명에 포함
        slope_str = f'slope{slope_values[0][0]}_{slope_values[0][1]}_{slope_values[1][0]}_{slope_values[1][1]}'
        exp_conditions = f'pop{int(pop_r1)}_{int(pop_r2)}_aged{aged_ratio_r1}_{aged_ratio_r2}_{slope_str}'
        filename=os.path.join(graphs_dir, f'fair_policy_epsilon_analysis_{filename_title}_{exp_conditions}.png')
        plt.savefig(f'{filename}', dpi=300, bbox_inches='tight')
        plt.close()

def plot_fair_policy_validation_alpha(df, graphs_dir, target_supply_rate=0.15):
    """
    Validation: Fair 정책에서 특정 supply_rate(alpha)일 때 Age Priority 모델의 
    전체 사망률을 epsilon에 따라 그래프로 그림
    
    Parameters:
    - df: 전체 데이터프레임
    - graphs_dir: 그래프 저장 디렉토리
    - target_supply_rate: 확인하고자 하는 supply_rate (기본값: 0.1)
    """
    # 'policy_type' 컬럼 생성
    df['policy_type'] = df['policy'].apply(lambda x: str(x).split('_')[0])

    # 'Fair' 정책이면서 'Age Priority' 배분만 필터링
    fair_age_priority_df = df[
        (df['policy_type'].str.lower() == 'fair') & 
        (df['allocation_type'] == 'Age Priority')
    ].copy()
    
    if fair_age_priority_df.empty:
        print("No 'Fair' policy with 'Age Priority' data found for validation.")
        return

    # 'epsilon'과 'supply_rate' 컬럼이 숫자인지 확인하고 NaN 값 처리
    fair_age_priority_df['epsilon'] = pd.to_numeric(fair_age_priority_df['epsilon'], errors='coerce')
    fair_age_priority_df['supply_rate'] = pd.to_numeric(fair_age_priority_df['supply_rate'], errors='coerce')
    fair_age_priority_df.dropna(subset=['epsilon', 'supply_rate'], inplace=True)

    # 특정 supply_rate(alpha) 데이터만 필터링
    target_df = fair_age_priority_df[
        fair_age_priority_df['supply_rate'] == target_supply_rate
    ].copy()
    
    if target_df.empty:
        print(f"No data found for supply_rate = {target_supply_rate}")
        available_rates = sorted(fair_age_priority_df['supply_rate'].unique())
        print(f"Available supply rates: {available_rates}")
        return
    
    # epsilon으로 정렬
    target_df = target_df.sort_values('epsilon')
    ...
    # 절대 사망률 그래프
    ax.plot(target_df['epsilon'], target_df['Total_Mortality'], 
            marker='o', markersize=8, linewidth=2, color='#e74c3c', label='Total Mortality')
    
    ax.set_title(f'Fair Policy Validation: Total Mortality vs. Epsilon\n(Age Priority, Supply Rate = {target_supply_rate})', 
                 fontsize=14, fontweight='bold')
    ax.set_xlabel('Epsilon (ε)', fontsize=12)
    ax.set_ylabel('Total Mortality Rate', fontsize=12)
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=11)
    
    # 추세선 추가
    z = np.polyfit(target_df['epsilon'], target_df['Total_Mortality'], 1)
    p = np.poly1d(z)
    ax.plot(target_df['epsilon'], p(target_df['epsilon']), 
    ...
    for idx, row in target_df.iterrows():
        ax.annotate(f'{row["Total_Mortality"]:.6f}', 
                   xy=(row['epsilon'], row['Total_Mortality']),
    ...
    slope_str = f'slope{slope_values[0][0]}_{slope_values[0][1]}_{slope_values[1][0]}_{slope_values[1][1]}'
    exp_conditions = f'pop{int(pop_r1)}_{int(pop_r2)}_aged{aged_ratio_r1}_{aged_ratio_r2}_{slope_str}'
    filename = os.path.join(graphs_dir, 
                           f'validation_fair_alpha{target_supply_rate}_total_mortality_{exp_conditions}.png')
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Validation graph saved: {filename}")
    print(f"Epsilon range: {target_df['epsilon'].min()} to {target_df['epsilon'].max()}")
    print(f"Mortality range: {target_df['Total_Mortality'].min():.6f} to {target_df['Total_Mortality'].max():.6f}")
    print(f"Trend: {'INCREASING' if z[0] > 0 else 'DECREASING'} (slope: {z[0]:.8f})")
...
def main_comparison_analysis():
    ...
    # 실험 조건을 파일명에 포함
    pop_str = f"pop{populations[0]}_{populations[1]}"
    aged_str = f"aged{aged_ratio[0]}_{aged_ratio[1]}"
    slope_str = f"slope{slope_values[0][0]}_{slope_values[0][1]}_{slope_values[1][0]}_{slope_values[1][1]}"
    exp_conditions = f"{pop_str}_{aged_str}_{slope_str}"
    
    aged_priority_file = os.path.join(output_dir, f'policy_results_with_priority_{exp_conditions}.csv')
    non_priority_file = os.path.join(output_dir, f'policy_results_without_priority_{exp_conditions}.csv')
    
    # 파일이 존재하지 않으면 기본 파일명으로 시도
    if not os.path.exists(aged_priority_file):
        print(f"Warning: {aged_priority_file} not found. Trying default filename...")
        aged_priority_file = os.path.join(output_dir, 'policy_results_with_priority.csv')
    if not os.path.exists(non_priority_file):
        print(f"Warning: {non_priority_file} not found. Trying default filename...")
    ...
    # Validation: Fair 정책 alpha=0.1일 때 epsilon에 따른 사망률 변화
    plot_fair_policy_validation_alpha(df_combined, graphs_dir, target_supply_rate=0.1)
